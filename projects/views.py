...
class StateUploadAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            required_columns = {'name', 'abbreviation'}
            if not required_columns.issubset(csv_reader.fieldnames):
                return Response({"error": "CSV file must contain 'name' and 'abbreviation' columns."}, status=status.HTTP_400_BAD_REQUEST)
            ...
            for row in csv_reader:
                ...
                state, created = State.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'slug': None,  # Let SlugMixin handle slug generation
                    }
                )
                states_created.append({"name": state.name, "created": created})

            return Response({"message": "File processed successfully.", "states": states_created}, status=status.HTTP_201_CREATED)
        except Exception as e:
